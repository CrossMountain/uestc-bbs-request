...
const { username, password, randomContent } = fixedConfig;
...
function getLoginOps(res) {
  ...
  //获取formhash
  const formhashArr = util.HTMLExtractAttr(
    res.data,
    'input[name="formhash"]',
    'value'
  );
  ...
  const afterLoginURL =
    baseURL +
    'member.php?mod=logging&action=login&loginsubmit=yes&loginhash=' +
    loginURLHash +
    '&inajax=1';
...
//获取formhash
function getFormhashOps(res, targetURL) {
...
function getPostOps(res) {
  const formActionValue = util.HTMLExtractAttr(
    res.data,
    '#fastpostform',
    'action'
  )[0];
  const formhash = util.HTMLExtractAttr(
    res.data,
    'input[name="formhash"]',
    'value'
  )[0];
